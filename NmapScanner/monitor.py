...
def checkhosts(namespace):
    ...
    for bucket in res["aggregations"]["2"]["buckets"]:
        #try:
        command = 'nmap -p22,443,8443,1434,2001,80,990,21,179,25 -Pn -sS ' + bucket[
            "key"] + ' --script "(http* and safe),(ssl*),(discovery),(vuln)" --script-timeout 5 -oX /tmp/nmapoutput.xml'
        print(command)
        os.system(command)
